...
class SKLearnTrainer:
	def __init__(
		...
		masked_train_X = torch.where(train_dataset.attention_masks.bool(), train_dataset.X,
									 torch.tensor(model.tokenizer.pad_token_id))
		train_documents = [' '.join(model.tokenizer.convert_ids_to_tokens(x, skip_special_tokens=True)) for x in
						   masked_train_X]
		...
		masked_eval_X = torch.where(eval_dataset.attention_masks.bool(), eval_dataset.X,
									torch.tensor(model.tokenizer.pad_token_id))
		eval_documents = [' '.join(model.tokenizer.convert_ids_to_tokens(x, skip_special_tokens=True)) for x in
						  masked_eval_X]
